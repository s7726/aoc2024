...
.........."""
    ...
    def test_new_gridpoint(self):
        self.assertEqual(new_gridpoints_from_start((0, 0), (2, 2)), [(2, 2), (-2, -2)])

    def test_gridpoints_all_in_direction(self):
        self.assertEqual(
            new_gridpoints_in_all_directions((0, 0), (1, 1), 2), [(1, 1), (-1, -1)]
        )
...
def new_gridpoints_in_all_directions(point, grid_distance, num_each_dir):
    new_points = []
    for i in range(1, num_each_dir):
        new_points.append(tuple_add(point, tuple_multiply(grid_distance, (i, i))))
        new_points.append(tuple_add(point, tuple_multiply(grid_distance, (-i, -i))))
    ...
    for freq, antennas in freqs.items():
        ...
        for antenna_pairing in antenna_pairings:
            ...
            for point in antenna_pairing:
                new_points = new_gridpoints_in_all_directions(
                    point, dist, len(node_mat)
                )
                new_points = [
                    p
                    for p in new_points
